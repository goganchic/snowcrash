...
namespace snowcrash
{
     ...
     */
    enum SectionLayout {
        DefaultSectionLayout, /// Default Section Layout: Signature > Description >
        /// Content > Nested
    ...
    struct SectionProcessorBase {
        ...
        /**
         * Pre-process for nested sections, basically a look-ahead before processing
         * nested sections
            ...
            const ParseResultRef<T>& out)
        {
            ...
            if (node->type == mdp::HeaderMarkdownNodeType) {
                ss << "unexpected header block, expected a group, resource or an action "
                      "definition";
                ss << ", e.g. '# Group <name>', '# <resource name> [<URI>]' or '# <HTTP "
                      "method> <URI>'";
            } else {
