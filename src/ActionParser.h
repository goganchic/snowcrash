...
namespace snowcrash
{
    ...
    const char* const NamedActionHeaderRegex
        = "^[[:blank:]]*" SYMBOL_IDENTIFIER "\\[" HTTP_REQUEST_METHOD "[[:blank:]]*" URI_TEMPLATE "?]$";
    ...
    struct SectionProcessor<Action> : public SectionProcessorBase<Action> {
        ...
        /**
         *  Checks whether given section payload has duplicate within its transaction
         * examples
            ...
            const ParseResultRef<Headers>& out)
        {
            ...
            std::stringstream ss;
            ss << "the 'headers' section at this level is deprecated and will be "
                  "removed in a future, use respective payload header section(s) "
                  "instead";
