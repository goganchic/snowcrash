            ...
            const ParseResultRef<mson::TypeSection>& out)
        {
            ...
            if (IEqual<std::string>()(signature.identifier, "Default")) {
                ...
                if (out.node.baseType != mson::ValueBaseType && out.node.baseType != mson::ImplicitValueBaseType) {
                    ...
                    ss << "` not allowed for a type sub-typed from a primitive or object "
                          "type";
            ...
            if (assignValues && (!signature.values.empty() || !signature.value.empty())) {
                ...
                if (out.node.baseType == mson::PrimitiveBaseType
                    ...
                    out.report.warnings.push_back(
                        Warning("a sample and/or default type section for a type which is "
                                "sub-typed from an object cannot have value(s) beside the keyword",
                            LogicalErrorWarning,
                            sourceMap));
                }
            }
