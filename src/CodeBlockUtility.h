...
namespace snowcrash
{
    ...
    struct CodeBlockUtility {
        ...
        /**
         *  \brief  Retrieve the textual content of a Markdown node as if it was a
         * code block.
            ...
            const MarkdownNodeIterator& node, const SectionParserData& pd, Report& report, mdp::ByteBuffer& content)
        {
            ...
            ss << " is expected to be a pre-formatted code block, every of its line "
                  "indented by exactly ";
            ...
            const MarkdownNodeIterator& node, const SectionParserData& pd, Report& report, mdp::ByteBuffer& content)
        {
            ...
            ss << " is expected to be a pre-formatted code block, separate it by a "
                  "newline and ";
            ...
            mdp::ByteBuffer& out)
        {
            ...
            if (level) {
                // WARN: Dangling asset
                std::stringstream ss;
                ss << "dangling message-body asset, expected a pre-formatted code "
                      "block, ";
        ...
        /**
         *  \brief Check for potential resource model reference and warn about not
         * recognizing them
         *  \return True if code block contains a resource model reference, false
         * otherwise.
         */
            ...
            const MarkdownNodeIterator& node, const SectionParserData& pd, Report& report)
        {
            ...
            if (GetModelReference(source, symbol)) {
                ...
                ss << "a reference must be directly in the " << SectionName(pd.sectionContext())
                   << " section, indented by 4 spaces or 1 tab, without any additional "
                      "sections";
